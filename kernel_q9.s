...
addi $2, $0, 1                  # Enable the PCB enabled flag
sw $2, pcb_enabled($1)

la $2, exitTask                 # Initialise the PCB return address field
sw $2, pcb_ra($1)
...
la $1, idletask_pcb             # Setup PCB for the IDLE task

la $2, idletask_pcb             # Setup the link field (with the next task)
sw $2, pcb_link($1)

la $2, idletask_stack           # Setup the stack pointer
sw $2, pcb_sp($1)

la $2, idleTask                 # Setup the $ear field
sw $2, pcb_ear($1)

addi $2, $0, 0                  # Initialise the PCB enabled flag to 0
sw $2, pcb_enabled($1)
...
add $3, $0, $0                  # Initialise $3 to 0 to use as a counter
checkEnabledLoop:               # Checks/ensures that the task that is about to be run is enabled using enabled flag in the PCB
lw $2, pcb_enabled($13)         # Get the value of the flag into $2
bnez $2, exitLoop               # If it is enabled, exit the loop
addi $3, $3, 1                  # Increment the counter that tracks the number of exited tasks
seqi $4, $3, 3                  # If counter is 3, all other tasks exited, so jump to run the IDLE task
bnez $4, idleTask
lw $13, pcb_link($13)           # Get the next task from the pcb_link field

j checkEnabledLoop              # Loop back around
...
idleTask:                       # Subroutine for a task that runs after all other tasks exited - It displays "----" to the 4 SSDs

la $1, idletask_pcb             # Set IDLE task as the current task
sw $1, current_task($0)

lw $13, current_task($0)        # Get the IDLE task from current_task
addi $1, $0, 1                  # Enable the PCB enabled flag for the IDLE task
sw $1, pcb_enabled($13)

sw $0, 0x73004($0)              # Disable hex-decode by writing 0 to the parallel control register
addi $13, $0, 0x40              # Set $13 to be the manual encoding of '-'
sw $13, 0x73009($0)             # Display '-' to each of the 4 SSDs
sw $13, 0x73008($0)
sw $13, 0x73007($0)
sw $13, 0x73006($0)


exitTask:                       # Subroutine that enables tasks to exit cleanly by removing current_task from the scheduling queue
lw $13, current_task($0)        # Get the current task
sw $0, pcb_enabled($13)         # Set the enabled flag for current task PCB to be 0
...
idletask_pcb:                   # Defines a process control block for the IDLE task
.space 20
...
.space 200                      # Defines a separate stack for the IDLE task
idletask_stack:   
